...
bfiles = c("hyp-miR-E12-A.bam", "hyp-miR-E12-B.bam", "hyp-miR-E20-A.bam", "hyp-miR-E20-B.bam")
fcmirnas <- featureCounts(files=bfiles, annot.ext="mm10-ensembl_99-genes.gtf", 
                    isGTFAnnotationFile=T, useMetaFeatures=T, minMQS=10, largestOverlap=T,
                    isPairedEnd=F, requireBothEndsMapped=T, nthreads=5)
## File save
count_data <- fcmirnas$counts
genes_data <- fcmirnas$annotation
head(genes_data)
...
## table counts
counts <- fcmirnas$counts

## dataprep
sample_names <- colnames(counts)
group <- factor(c("FAD", "FAD", "WT", "WT"))
genes <- fcmirnas$annotation
